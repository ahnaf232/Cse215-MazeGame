...
public class Maze {
    ...
    /**
      Determines if your character can move
      return true if there are no obstacles to the right left up or down.
     */
    ...
    /**
     * Moves your character one space right left up or down.
     */
    ...
    /* private void fillSolution() {
        ...
        // Randomly generate the solution path
        int currentRow = 1;
        int currentCol = 0;
        solution[currentRow][currentCol] = '*';

        while (currentRow != 10 || currentCol != 19) {
            int direction = rand.nextInt(4);
            switch (direction) {
                case 0: // move right
                    if (currentCol < 19 && solution[currentRow][currentCol + 1] == '.') {
                        currentCol++;
                    }
                    break;
                case 1: // move left
                    if (currentCol > 0 && solution[currentRow][currentCol - 1] == '.') {
                        currentCol--;
                    }
                    break;
                case 2: // move down
                    if (currentRow < 19 && solution[currentRow + 1][currentCol] == '.') {
                        currentRow++;
                    }
                    break;
                case 3: // move up
                    if (currentRow > 0 && solution[currentRow - 1][currentCol] == '.') {
                        currentRow--;
                    }
                    break;
            }
        ...
        // Add random walls
        for (int i = 0; i < 50; i++) {
            int wallRow = rand.nextInt(20);
            int wallCol = rand.nextInt(20);
            if (solution[wallRow][wallCol] == '.') {
                solution[wallRow][wallCol] = '-';
            }
        }

        addPits(); // Call addPits after setting up the maze
        printMap(solution);
    }*/
       public void addPits() {
    int numberOfPits = 10; 
